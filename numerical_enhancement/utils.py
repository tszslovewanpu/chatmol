...
# model_kwargs.items()
# dict_items([('input_ids', tensor([[128001, 128001, 128001, 128001, 128001, 128001, 128001, 128001, 128001,
...
### biubiubiu  
# model_kwargs['input_ids'][0].shape
# torch.Size([88])
# model_kwargs['input_ids'][1].shape
# torch.Size([88])
...
# {}
        ...
        if not self.config.is_encoder_decoder and model_input_name == "inputs_embeds":
            ...
            model_kwargs["use_cache"] = generation_config.use_cache # here
...
###### biubiubiu
            ...
            result = self._sample(
                input_ids,
                numbers=numbers, ###
                numbers_start_idx=numbers_start_idx, ###
                numbers_end_idx=numbers_end_idx, ###
                logits_processor=prepared_logits_processor,
...
#           48983,    320,   1861,  36660,   3931,   2891,     25]],
#        device='cuda:0')
# input_ids[0][numbers_start_idx[0]:numbers_end_idx[0]+1]
# tensor([ 482,   17,   13, 1135], device='cuda:0')
# input_ids[1][numbers_start_idx[1]:numbers_end_idx[1]+1]
# tensor([ 482,   17,   13, 1135], device='cuda:0')
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
### 这一步开始，不结束就一直返回这里送入模型，这里调用了模型的forward方法！！！biubiubiu ### fcl stepp # fcl step # fcl
            # forward pass to get next token
            outputs = self(
                **model_inputs,
